...
# model = keras.models.load_model('./save_model/life_10_224_30')  # 加载训练好的完整模型
model = keras.models.load_model('./save_model/life_18_224_10.h5') # 加载训练好的完整模型
...
def get_img_pathlist(data_dir='./life_10/validation', image_name='roses'):
    """
    批量获取图片
    :param data_dir:
...
def main_test_image(image_name, data_dir):
    ...
    if len(error_img_list) > 0:
        test_res = '{0} 批量识别准确率：{1} %'.format(image_name, str(1 - len(error_img_list) / len(image_path_list))[:4])
    else:
...
if __name__ == '__main__':
    ...
    img_height = 224
    img_width = 224
    class_names = ['公交车', '女士', '小汽车', '小狗', '小猫', '手机', '显示器', '椅子', '水杯', '火车', '男士', '眼镜', '自行车', '船', '键盘', '飞机', '饮料', '鼠标']  # 训练时类型 顺序必须和文件夹顺序一致

    # 识别
    # data_dir = ['./img/302_20201103_1107_24801384.jpg']
    # data_dir = get_img_pathlist(data_dir='./life_18/validation', image_name='bike') # 通上一次获取所有验证图
    data_dir = './life_18/training'  # training
    all_list = []
