...
train_dir = './life_18/training'
valid_dir = './life_18/validation'
lable_file = './life_18/lable.txt'
...
train_datagen = keras.preprocessing.image.ImageDataGenerator(
    preprocessing_function=keras.applications.resnet50.preprocess_input, # 专门为resnet50设计的图像处理函数带有归一化，只在keras中，不在tf.keras中
                                                             rotation_range=50, # 将图像随机旋转
                                                             width_shift_range=0.3, # 水平方向 处理图像位移，在0-20%随机选择数
                                                             height_shift_range=0.3, # 它们0-1之间的数是比例， 大于1是像素值
...
epochs = 40 # fine_tune 10次就可以达到比较好的效果

# 恢复模型后，您可以继续在它上面进行训练，甚至无需再次调用 compile()，因为在保存之前已经对其进行了编译。模型以 TensorFlow 的标准 SavedModel proto 格式保存
resnet50_fine_tune = keras.models.load_model('./save_model/life_18_224_60.h5')
